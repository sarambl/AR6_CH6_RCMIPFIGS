...
# %%
TABLE_DIR = RESULTS_DIR / 'tables_old_IRF/'
...
vn_sum = 'old_IRF_Sum SLCF (%s)' % _str[:-2]
print(vn_sum)
...
for var in variables_erf_comp:
    ...
    _st = f'Old_IRF_{var}_deltaT_time.csv'
...
def fign_dt(_var, _s_y, _e_y):
    return 'old_IRF_%s_refy%s_fy%s.png' % (_var.replace(' ', '_').replace('|', '-'), _s_y, _e_y)

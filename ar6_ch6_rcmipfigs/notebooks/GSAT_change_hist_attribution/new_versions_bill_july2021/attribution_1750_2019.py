...
#First scale to CH 7 2014, but only scale scattering aerosols
#Modify Ch 7 2014 ari by subtracting off Thornhill ari_BC
irf_ari_scat = irf_ari-ari[i_bc] # Set non-BC ari to 7.3.3

# scale SO2+OC+NH3 to get total scattering ari as Ch 7
# This (inconsistently) doesn't include ari from gases
ari_scat = np.sum(ari[i_scat]) #Sum is only over scattering
ari[i_scat] = ari[i_scat]*irf_ari_scat/ari_scat 
ari_sd[i_scat] = ari_sd[i_scat]*irf_ari_scat/ari_scat

# For aci scaling include BC component too
...
#This now includes ari from gas precursors (inconsistent with 2014 scaling)
# - and includes BC
ari_sd *= ari_erf_AR6/np.sum(ari) # Sum is over all emitted species
ari *= ari_erf_AR6/np.sum(ari)

#Aci scaling is over all aerosols including BC

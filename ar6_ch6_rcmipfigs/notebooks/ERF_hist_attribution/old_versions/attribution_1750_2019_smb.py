...
# %%
    ...
    dtype = 'U12'+', f8'*ncols
    data = np.genfromtxt('../../../data_in/input_from_bill_collins/attribution_input.csv', delimiter=',', filling_values=0,
                         names=True, dtype=(dtype))
    data_sd = np.genfromtxt('../../../data_in/input_from_bill_collins/attribution_input_sd.csv', delimiter=',', filling_values=0,
                            names=True, dtype=(dtype))
    data_fo3 = data['o3_rf']
    data_rfo3_sd = data_sd['o3_rf_sd']
    data_lifech4 = data['lifech4']
    data_lifech4_sd = data_sd['lifech4_sd']
    data_ari = data['ari']
    data_ari_sd = data_sd['ari_sd']
    data_ac = data['ac']
    data_ac_sd = data_sd['ac_sd']
    data_erf = data['erf']
    data_erf_sd = data_sd['erf_sd']
    ...
    ch4 = ch4_2014*(1+data_lifech4)**alpha
    ch4_sd = (ch4-ch4_2014)*data_lifech4_sd/data_lifech4
    ch4_sd = np.where(data_lifech4 == 0, 0., ch4_sd)

    # Ozone primary mode
    rfo3perch4 = data_fo3[i_ch4]/(ch4_2014-ch4_1850) # Use CH4 expt
    rfo3perch4_sd = data_rfo3_sd[i_ch4]/(ch4_2014-ch4_1850) # Use CH4 expt
    ...
    data_ari[i_aer] = data_erf[i_aer]-data_ac[i_aer]
    data_ari_sd[i_aer] = np.sqrt(data_erf_sd[i_aer]**2 +data_ac_sd[i_aer]**2)

    # scale SO2+OC to get total ari

    irf_ari_scat = irf_ari-data_ari[i_bc] # Set non-BC ari to 7.3.3 FGD
    ari_scat = np.sum(data_ari[i_scat])
    data_ari[i_scat] = data_ari[i_scat]*irf_ari_scat/ari_scat
    data_ari_sd[i_scat] = data_ari_sd[i_scat]*irf_ari_scat/ari_scat

    # scale aci to get total aci from 7.3.3
    total_aci = np.sum(data_ac[i_aer])
    data_ac[i_aer] = data_ac[i_aer]*erf_aci/total_aci
    data_ac_sd[i_aer] = data_ac_sd[i_aer]*erf_aci/total_aci
    #############################################################
    # Aerosol end
    #############################################################
    ...
    data_fo3    *= o3_erf_AR6/np.sum(data_fo3)
    data_rfo3_sd *= o3_erf_AR6/np.sum(data_fo3)

    data_ari *= ari_erf_AR6/np.sum(data_ari)
    data_ari_sd *= ari_erf_AR6/np.sum(data_ari)

    data_ac[i_aer] *= aci_erf_AR6/np.sum(data_ac[i_aer])
    data_ac_sd[i_aer] *= aci_erf_AR6/np.sum(data_ac[i_aer])
    ...
    for ispec in np.arange(nspec):
        ...
        table['O3'][ispec+1] = data_fo3[ispec]
        table['O3_prime'][ispec+1] = rfo3_prime[ispec]
        table['Aerosol'][ispec+1] = data_ari[ispec]
        table['Cloud'][ispec+1] = data_ac[ispec]
        table['Total'][ispec+1] = np.sum([rfco2[ispec], rfghg[ispec], rfch4[ispec],
             data_fo3[ispec], rfo3_prime[ispec], data_ari[ispec], data_ac[ispec]])
        ...
        table_sd['O3_sd'][ispec+1] = data_rfo3_sd[ispec]
        table_sd['O3_prime_sd'][ispec+1] = rfo3_prime_sd[ispec]
        table_sd['Aerosol_sd'][ispec+1] = data_ari_sd[ispec]
        table_sd['Cloud_sd'][ispec+1] = data_ac_sd[ispec]
        table_sd['Total_sd'][ispec+1] = np.sqrt(np.sum(np.square(
                [rfco2[ispec]*0.12, rfghg_sd[ispec], rfch4_sd[ispec],
                 data_rfo3_sd[ispec]+rfo3_prime_sd[ispec], data_ari_sd[ispec], data_ac_sd[ispec]])))
    ...
    table_sd['Total_sd'][i_ch4+1] = np.sqrt(np.sum(np.square(
            [rfco2[i_ch4]*0.12, rfghg_sd[i_ch4]+rfch4_sd[i_ch4],
                 data_rfo3_sd[i_ch4]+rfo3_prime_sd[i_ch4], 0.05,
                 data_ari_sd[i_ch4], data_ac_sd[i_ch4]])))
    ...
    pos_o3[i_gas+1] = pos_ch4[i_gas+1]+\
        np.maximum(data_fo3[i_gas]+rfo3_prime[i_gas], 0.)
    neg_o3[i_gas+1] = neg_ch4[i_gas+1]+\
        np.minimum(data_fo3[i_gas]+rfo3_prime[i_gas], 0.)
    pos_h2o[i_gas+1] = pos_o3[i_gas+1]
    pos_h2o[i_ch4+1] += 0.05 # AR6 FGD
    pos_aer[i_gas+1] = pos_h2o[i_gas+1]+\
        np.maximum(data_ari[i_gas], 0.)
    neg_aer[i_gas+1] = neg_o3[i_gas+1]+\
        np.minimum(data_ari[i_gas], 0.)
    pos_cloud[i_gas+1] = pos_aer[i_gas+1]+\
        np.maximum(data_ac[i_gas], 0.)
    neg_cloud[i_gas+1] = neg_aer[i_gas+1]+\
        np.minimum(data_ac[i_gas], 0.)

    #Aerosols
    pos_aer[i_aer+1] = np.maximum(data_ari[i_aer], 0.)
    neg_aer[i_aer+1] = np.minimum(data_ari[i_aer], 0.)
    pos_cloud[i_aer+1] = pos_aer[i_aer+1]+\
        np.maximum(data_ac[i_aer], 0.)
    neg_cloud[i_aer+1] = neg_aer[i_aer+1]+\
        np.minimum(data_ac[i_aer], 0.)


    error = np.zeros(nspec+1)
    error[0] = co2[0]*0.12 # 12% uncertainty
    error[i_ch4+1] = np.sqrt((rfghg_sd[i_ch4]+rfch4_sd[i_ch4])**2+ # CH4
                        (data_rfo3_sd[i_ch4]+rfo3_prime_sd[i_ch4])**2+  # O3
                        0.05**2+                                   # Strat H2O
                        data_ari_sd[i_ch4]**2+
                        data_ac_sd[i_ch4]**2)
    error[i_non_ch4+1] = np.sqrt(rfghg_sd[i_non_ch4]**2+
                        rfch4_sd[i_non_ch4]**2+
                        (data_rfo3_sd[i_non_ch4]+rfo3_prime_sd[i_non_ch4])**2+
                        data_ari_sd[i_non_ch4]**2+
                        data_ac_sd[i_non_ch4]**2)
    error[i_aer+1] = np.sqrt(data_ari_sd[i_aer]**2+
                        data_ac_sd[i_aer]**2)
    ...
    #plt.barh(ybar, pos_ghg-pos_co2, width, left=pos_co2, color=get_chem_col('WM)'darkred', label=labels[1])
